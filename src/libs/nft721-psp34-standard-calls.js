...
async function mintWithAttributes(
  ...
  console.log('mintWithAttributesmintWithAttributes')
  console.log(gasLimit)
  // metadata = {
  //   name: "My NFT",
  //   description: "This is my NFT"
  // };
  const metadataArray = Object.entries(metadata).map(([name, value]) => [name, value]);
  const metadataType = api.createType("Vec<(Text, Text)>", metadataArray);
    ...
    .signAndSend(address, { signer }, async ({ status, dispatchError }) => {
      ...
      if (status?.isFinalized) {
        ...
        if (attributes?.length) {
          ...
          console.log('cacheImages.lengthcacheImages.length')
          console.log(cacheImages.length)
